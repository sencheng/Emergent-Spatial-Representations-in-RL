...
def single_run():
    ...
    if action_space == 'Egocentric' :
        ...
        modules['spatial_representation'] = HexagonalGraphAllocentric(n_nodes_x=9, n_nodes_y=9,
                                                   ...
                                                   observation_module=modules['observation'], 
                                                   rotation=True)
    n_nodes  = len(modules['spatial_representation'].nodes)
    np.random.seed(random_seed)
    start_locs = np.arange(0, n_nodes)
    train_on = np.random.choice(start_locs,int(start_locs.size*0.8))
    modules['spatial_representation'].set_start_nodes(train_on)    
