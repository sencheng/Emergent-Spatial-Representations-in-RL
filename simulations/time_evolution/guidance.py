...
if action_space == "Egocentric" : 
    save_path = 'guidance_egocentric'
    network = "../../networks/egocentric_network.json"
else : 
    save_path = 'guidance_allocentric'
...
def single_run():
    ...
    def record_activations(logs) :
        trial    = logs['trial']
        rl_agent = logs['rl_parent']
        if not os.path.exists(ts) :
            os.mkdir(ts)
        dense_layer_activations = K.function([rl_agent.model.layers[0].input],
                                             [rl_agent.model.layers[8].output])
        activations = []
        
        if trial%record_interval == 0:
            for angle in input_img : 
                for im in angle : 
                    activations.append(dense_layer_activations([[[im]]]))
            np.save('%s/%s.npy'%(ts,trial),activations)
    ...
    rl_agent.agent.save_weights(save_path + '/' + weights_file)    
    #save training data
    np.save(save_path + '/'+ training_file, data)
    shutil.move(ts, save_path + '/'+ ts)
    # stop simulation
